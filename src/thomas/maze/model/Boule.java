...
public class Boule {
	...
	private void isWon() {;
		...
		if (RectF.intersects(rect, endCase)){
			double distance = Math.sqrt((endCase.centerX() - rect.centerX()) * (endCase.centerX() - rect.centerX()) + (endCase.centerY() - rect.centerY()) * (endCase.centerY() - rect.centerY()));
	...
	private boolean collide2() {
		if (wallsNHoles == null)
			return false;
		ArrayList<Case> wallsNHolesTmp = (ArrayList<Case>) wallsNHoles.clone();
		for (Case c : wallsNHolesTmp) {
